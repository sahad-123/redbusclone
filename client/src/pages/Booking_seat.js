...
function Booking_seat() {
    ...
    /*    nam=e.target.name;
        ...
        console.log("index:",index,"nam:",key,"value:",value);
        setBookings({...bookings,[key]:value});
     
     
        console.log("bookings:",index,bookings);
        }
    /*
...
*/
        const  addBooking=async(id)=>{
           ...
           schedule_id:schedule_id
                  ...
                  const res = await fetch('/obtain_schedule_information',{
                     method:"POST",
                     headers:{
        ...
        console.log("data:",data.schedule_infor);
        let passenger_name=bookings.Name;
        let Male=bookings.Male;
        let Female=bookings.Female;
        let age=bookings.age;
        let city_of_residence=bookings.city_residence;
        let city_of_state=bookings. state_residence;
        let total_price=price;
        let deprature_date=id3;
  ...
  let boording_point=bor;

  let dropping_point=dro;
  


        let start_time=data.schedule_infor.hour_and_miniute_starting_time_string;

        let estimated_time=data.schedule_infor.estimated_time;
...
console.log(  passenger_name,
  ...
  Female,
   age,
 city_of_residence,
  city_of_state,"<= passenger_name Male,Female age,city_of_residence,city_of_state")
       ...
       let s8={
        ...
        estimated_time:estimated_time
                 ...
                 const res = await fetch('/Bus_booking_information_sending',{
                    method:"POST",
                    headers:{
             ...
             navigate(`/Booking_sucess/${schedule_id}/${seat_count}/${bor}/${dro}/${seat_ids}/${id3}/${id2}/${price}/${countr1}/${userid}`);
           
        }

    let {schedule_id,seat_count,bor,dro,seat_ids,id3,id2,price,countr1,userid} = useParams();
      
   // Assuming seat_count is passed as a query parameter
    const price1 = parseFloat(price)
    
    
    
    
    let total_price=countr1 * price1;
console.log("total_price:",total_price);
   let seat_ids1;

     seat_ids1=seat_ids.split('&');
console.log(" seat_ids1:", seat_ids1);
    ...
    let elements = [];
        ...
        if(seat_count!=0){
              ...
              elements.push(
                ...
                <div  className='show  d-flex  flex-column gap12' key={seat_count}>
         
                <div>
        <input   onChange={(e) => handleInputChange(seat_count  , e.target.name, e.target.value)} name="Name"  type='text' placeholder='Name' />
                </div>
                
        
                <div>
                <input   onChange={(e) => handleInputChange(seat_count, e.target.name, e.target.value)}  name="Male" type='text' placeholder='IF PASSENGER IS MALE THEN TRUE ' />
             
                </div>
                <div>
                <input  onChange={(e) => handleInputChange(seat_count, e.target.name, e.target.value)}  name="Female" type='text' placeholder='IF PASSENGER IS FEMALE THEN FALSE' />
                </div>
        
        
         
                <div>
        <input  onChange={(e) => handleInputChange(seat_count, e.target.name, e.target.value)}  type='number'   name="age"    placeholder='age' />
                </div>
                
        
                <div>
                <input   onChange={(e) => handleInputChange(seat_count, e.target.name, e.target.value)}   name="city_residence"  type='text'  placeholder='city of residence'/>
        
                </div>
                <div>
        
                <input    onChange={(e) => handleInputChange(seat_count, e.target.name, e.target.value)}  name="state_residence" type='text'  placeholder='state of residence'/>
                </div>
        <div>
        
        <button  onClick={()=>addBooking(seat_ids1[seat_count])}>PASSENGER  {seat_ids1[seat_count]}</button>
        </div>
...
//callnavigate();
  ...
  return (
   seat_count>0 ?(  <div   className='booking_show  d-flex    gap5i '>
    ...
    {elements}
      ...
      <div class="screen">
        ...
        <p class="message">BUS BOOKING SUCESSFULL</p>
        <Link  className='done' to={`/payment1/${schedule_id}/${seat_count}/${bor}/${dro}/${seat_ids}/${id3}/${id2}/${price}/${countr1}/${total_price}/${userid}`}><h2 class="button">Done</h2></Link>
      </div>
    </div>
